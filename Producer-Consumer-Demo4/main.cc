 ...
 * author: Martin
 * date: Fri Feb 1 2019
 * description: multi producers and multi consumers
 * 
 ********************************************************/
...
void ProducerTask(void)
{
    ...
    while (1) {
        sleep(1);
        std::unique_lock<std::mutex> lock(g_itemRepository.mtx_produced_item_counter);
        if (g_itemRepository.produced_item_counter < ITEM_TO_PRODUCE) {
            ProduceItem(&g_itemRepository, ++g_itemRepository.produced_item_counter);
            std::cout << "Producer thread ID: " << std::this_thread::get_id() << ". Produce the " << g_itemRepository.produced_item_counter << "^th item..." << std::endl; std::fflush(stdout);
        }
        else {
            ready_to_exit = true;
        }
...
void ConsumerTask(void)
{
    ...
    while(1) {
        ...
        std::unique_lock<std::mutex> locking(g_itemRepository.mtx_consumed_item_counter);
        if (g_itemRepository.consumed_item_counter < ITEM_TO_PRODUCE) {
            int item = ConsumeItem(&g_itemRepository);
            g_itemRepository.consumed_item_counter++;
...
void InitItemRepository(ItemRepository *itemRepository)
{
    ...
    itemRepository->write_position = 0;
    itemRepository->read_position = 0;
    itemRepository->produced_item_counter = 0;
    itemRepository->consumed_item_counter = 0;
}
